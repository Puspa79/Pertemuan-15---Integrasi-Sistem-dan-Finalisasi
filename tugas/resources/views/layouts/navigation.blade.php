<nav x-data="{ open: false }" class="bg-white shadow-sm border-b border-gray-200">
    <div class="max-w-full mx-auto px-6 lg:px-10">
        <div class="flex justify-between h-20">
            ...
            {{-- Global Search & User Dropdown (Desktop) --}}
            <div class="hidden sm:flex sm:items-center sm:gap-5">
                
                {{-- Form Global Search Desktop --}}
                <form action="{{ route('search.global') }}" method="GET" class="relative text-gray-600 focus-within:text-gray-400">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <i class="bi bi-search text-gray-400"></i>
                    </span>
                    <input type="search" name="q" value="{{ request('q') }}" 
                        class="py-2 text-sm bg-gray-50 rounded-xl pl-9 pr-4 w-60 focus:outline-none focus:bg-white focus:text-gray-900 border border-gray-200 focus:border-blue-500 transition-all duration-150 placeholder-gray-400" 
                        placeholder="Cari buku atau anggota..." autocomplete="off">
                </form>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-2 pb-3 px-4 border-b border-gray-100">
            <form action="{{ route('search.global') }}" method="GET" class="relative text-gray-600">
                <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <i class="bi bi-search text-gray-400"></i>
                </span>
                <input type="search" name="q" value="{{ request('q') }}" 
                    class="py-2 text-sm bg-gray-50 rounded-xl pl-9 pr-4 w-full focus:outline-none focus:bg-white focus:text-gray-900 border border-gray-200 focus:border-blue-500 transition-all duration-150 placeholder-gray-400" 
                    placeholder="Cari buku atau anggota..." autocomplete="off">
            </form>
        </div>
