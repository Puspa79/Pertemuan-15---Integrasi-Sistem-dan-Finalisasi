<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-[90%] mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div
                class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                <div>
                    ...
                    <p class="text-base text-gray-500 mt-1">Kelola data peminjaman dan pengembalian buku perpustakaan.
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div
                    class="bg-white p-6 rounded-lg shadow-sm border-l-4 border-amber-500 flex flex-col justify-between">
                    <span class="text-sm font-bold text-gray-400 uppercase tracking-wider">Sedang Dipinjam</span>
                    <span
                        class="text-4xl font-extrabold text-amber-500 mt-2">{{ $transaksis->where('status', 'Dipinjam')->count() }}</span>
                </div>
                <div
                    class="bg-white p-6 rounded-lg shadow-sm border-l-4 border-green-500 flex flex-col justify-between">
                    <span class="text-sm font-bold text-gray-400 uppercase tracking-wider">Sudah Dikembalikan</span>
                    <span
                        class="text-4xl font-extrabold text-green-600 mt-2">{{ $transaksis->where('status', 'Dikembalikan')->count() }}</span>
                </div>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider w-16">No
                                </th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Kode</th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Anggota
                                </th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Buku</th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Tgl
                                    Pinjam</th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Tgl
                                    Kembali</th>
                                <th class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Status
                                </th>
                                <th
                                    class="px-6 py-4 text-sm font-bold text-gray-500 uppercase tracking-wider text-center w-32">
                                    Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @forelse($transaksis as $transaksi)
                                <tr class="hover:bg-gray-50/80 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap text-base text-gray-600">
                                        {{ $loop->iteration }}</td>
                                    <td
                                        class="px-6 py-4 whitespace-nowrap text-base font-mono text-pink-600 bg-pink-50/50 rounded px-1.5 py-0.5 inline-block my-3 mx-2">
                                        {{ $transaksi->kode_transaksi }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-base font-semibold text-gray-900">
                                        {{ $transaksi->anggota->nama }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-base text-gray-600">
                                        {{ $transaksi->buku->judul }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-base text-gray-500">
                                        {{ $transaksi->tanggal_pinjam->format('d M Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-base text-gray-500">
                                        {{ $transaksi->tanggal_kembali->format('d M Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-base">
                                        <div class="flex flex-col gap-1">
                                            @if ($transaksi->status == 'Dipinjam')
                                                <span
                                                    class="px-2 py-1 text-sm font-semibold rounded-full bg-amber-100 text-amber-800 w-fit">
                                                    Dipinjam
                                                </span>
                                            @else
                                                <span
                                            ...
                                            @if ($transaksi->status == 'Dipinjam' && \Carbon\Carbon::parse($transaksi->tanggal_kembali)->isPast())
                                                <span
                                                    class="px-2 py-1 text-sm font-semibold rounded-full bg-red-100 text-red-700 w-fit">
                                                    ⚠ Terlambat
                                                    {{ abs((int) now()->diffInDays(\Carbon\Carbon::parse($transaksi->tanggal_kembali))) }}
                                                    Hari
                                                </span>
                                            @endif
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8"
                                        class="px-6 py-10 text-center text-base text-gray-400 bg-gray-50/30">
